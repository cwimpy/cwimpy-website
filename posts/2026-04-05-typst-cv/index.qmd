...
description: "A walkthrough of building a data-driven academic CV using Quarto and Typst---with publications pulled automatically from a BibTeX file, custom styling, and a clean design that rivals InDesign."
image: "cv-preview.png"
format:
  html: default
  typst:
    output-file: "typst-cv-tutorial-wimpy.pdf"
format-links:
  - html
  - format: typst
    title: PDF
...
Compare this to the LaTeX equivalent, which typically involves `\begin{tabular}`, manual `\hfill` calls, and careful spacing. In Typst, the function handles all of that.
...
The key line is `hanging-indent: 2em`---Typst handles this natively. In LaTeX, you would need something like `\begin{hangparas}{2em}{1}` from the `hanging` package, or manual `\hangindent` and `\hangafter` commands.
