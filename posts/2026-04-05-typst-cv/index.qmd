...
└── _quarto.yml          # Project config (post-render cleanup)
```

The .qmd file contains the CV content and an R chunk that reads the BibTeX file. The .typ file defines the visual design (e.g., fonts, colors, spacing, and custom functions for entries). The .bib file is the same one I use for my website and papers. One source of truth for publications, used everywhere. The `_quarto.yml` file handles a small annoyance: Quarto generates a mediabag folder (`my-cv_files/`) during rendering that is not needed after the PDF is built. A post-render hook takes care of it:
...
This produces a clean two-line entry with the title and location on the first line (justified left and right), and the organization and dates on the second. The amount parameter is optional, but I use it for grants:
...
One style note: for conference and invited presentation titles, I wrap them in typographic curly quotes (\u201c \u201d) in the .qmd source to follow standard academic citation conventions for shorter works. Grant titles and employment entries remain unquoted. Since the template renders titles in bold, the quotes provide a visual cue that distinguishes presentation titles from section labels or position titles.
...
The key line is hanging-indent: 2em, as Typst handles this natively. In LaTeX, you would need something like the hangparas environment from the hanging package, or manual hangindent and hangafter commands.
...
The Typst template is designed to be modifiable. Change the colors at the top, swap the fonts, and adjust the spacing as needed. It is all in one file with clear variable names.

Typst is still young, but Posit's investment in Quarto integration signals that it has a real future in academic publishing. For CVs, at least in my experience so far, it is already beating LaTeX in almost every way that matters.
